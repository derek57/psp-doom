...
void WI_drawLF(void)
{
    ...
    {
        // draw <LevelName> 
        V_DrawPatch((SCREENWIDTH - SHORT(lnames[wbs->last]->width))/2, 0, 
                    y, lnames[wbs->last]);

        // draw "Finished!"
        y += (5*SHORT(lnames[wbs->last]->height))/4;

        V_DrawPatch((SCREENWIDTH - SHORT(finished->width)) / 2, y, 0, finished);
    }
...
void WI_drawStats(void)
{
    ...
    if (wbs->epsd < 3)
    {
	V_DrawPatch(SCREENWIDTH/2 + SP_TIMEX, SP_TIMEY, 0, par);
